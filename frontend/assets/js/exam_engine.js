...
// ─── IMAGE URL HELPER ────────────────────────────────────────────────────────
// Strips leading "uploads/" or "uploads\" so the URL always becomes
// /static/uploads/<basename> which FastAPI serves correctly.
function _imgUrl(path) {
  if (!path) return null;
  const clean = path.replace(/^uploads[/\\]/, '');
  return `/static/uploads/${clean}`;
}

// Render one question/solution image with proper sizing and error fallback.
function _imgTag(path, extraStyle) {
  const url = _imgUrl(path);
  if (!url) return '';
  return `<div style="margin:10px 0;text-align:center">
    <img src="${url}"
      style="max-width:100%;width:auto;height:auto;max-height:340px;
             border-radius:8px;border:1px solid var(--c-border);
             display:block;margin:0 auto;${extraStyle||''}"
      onerror="this.parentElement.innerHTML='<div style=\\'padding:6px 10px;background:var(--c-surface2);border-radius:6px;font-size:11px;color:var(--c-text4)\\'>Image unavailable: ${path.split('/').pop()}</div>'"
      alt="Question image">
  </div>`;
}

// ─── PRE-EXAM LAUNCH MODAL ──────────────────────────────────────────────────
async function examLaunch(opts) {
  ...
  <div style="padding:2px 0">
    ...
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
      ...
      ${isNEET ? `
      <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border:1.5px solid var(--c-border);border-radius:var(--radius);cursor:pointer" id="neet-layout-opt">
        <input type="radio" name="layout" value="neet" style="margin-top:2px;accent-color:var(--c-blue)"
          onchange="document.getElementById('neet-layout-opt').style.borderColor='var(--c-blue)';document.getElementById('neet-layout-opt').style.background='var(--c-blue-l)';document.getElementById('std-layout-opt').style.borderColor='var(--c-border)';document.getElementById('std-layout-opt').style.background='none'">
        <div><div style="font-size:12px;font-weight:700;color:var(--c-text)">NEET Book Layout (Offline-style)</div>
        <div style="font-size:11px;color:var(--c-text4)">Left: Printed question paper. Right: OMR sheet.</div></div>
      </label>` : `
      <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border:1.5px solid var(--c-border);border-radius:var(--radius);cursor:pointer" id="jee-layout-opt">
        <input type="radio" name="layout" value="jeeomr" style="margin-top:2px;accent-color:var(--c-blue)"
          onchange="document.getElementById('jee-layout-opt').style.borderColor='var(--c-blue)';document.getElementById('jee-layout-opt').style.background='var(--c-blue-l)';document.getElementById('std-layout-opt').style.borderColor='var(--c-border)';document.getElementById('std-layout-opt').style.background='none'">
        <div><div style="font-size:12px;font-weight:700;color:var(--c-text)">JEE OMR Mode (Offline-style)</div>
        <div style="font-size:11px;color:var(--c-text4)">Left: Question paper (read-only). Right: JEE OMR sheet.</div></div>
      </label>`}
    </div>
...
window._preExamStart = async function() {
  ...
  closeModal();
  await _examLoad({ ...window._preExamOpts, camera: cam, mic: mic,
    neet_mode: layout === 'neet', jee_omr_mode: layout === 'jeeomr' });
};
...
async function _examLoad(opts) {
  ...
  try {
    ...
    window._ES = {
      id: attempt.id, qs: questions, answers, cur: 0, qTimer: Date.now(),
...
function _examBuild(title) {
  ...
  const timer = ro
    ? `<span style="background:rgba(255,255,255,.12);color:rgba(255,255,255,.85);padding:4px 14px;border-radius:99px;font-size:11px;font-weight:700">REVIEW</span>`
  ...
  return `<div style="flex:1;display:grid;grid-template-columns:1fr 400px;overflow:hidden;min-height:0">
    ...
    <div style="display:flex;flex-direction:column;overflow:hidden;background:#fff9e6">
      <div style="background:#1a1a2e;color:#e0e0ff;padding:8px 14px;font-size:11px;font-weight:800;letter-spacing:.08em;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
        ...
        <div style="display:flex;gap:6px"><span id="omr-timer-mini" style="font-size:12px;font-weight:900;color:#fbbf24;font-family:monospace">--:--</span>
          <button onclick="window.print()" style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);color:#fff;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;cursor:pointer">Print</button>
        </div>
      </div>
...
function _buildJeeOMR(ro) {
  return `<div style="flex:1;display:grid;grid-template-columns:1fr 360px;overflow:hidden;min-height:0">
    ...
    <div style="display:flex;flex-direction:column;overflow:hidden;background:#f8fafc">
      <div style="background:#0f172a;color:#94a3b8;padding:8px 14px;font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
  ...
  sOrd.forEach(subj => {
    ...
    bySubj[subj].forEach(q => {
      ...
      h += `<div style="break-inside:avoid;margin-bottom:14px;padding-bottom:10px;border-bottom:1px dashed ${isNEET ? '#c8b06a' : '#cbd5e1'}">
        ...
        ${q.question_text ? `<div style="font-size:12px;color:${txtClr};margin-bottom:6px;line-height:1.6">${q.question_text}</div>` : ''}
        ${q.question_image_path ? `<img src="${_imgUrl(q.question_image_path)}" style="max-width:100%;height:auto;border-radius:4px;margin-bottom:6px;border:1px solid ${isNEET ? '#c8b06a' : '#cbd5e1'}" onerror="this.style.display='none'">` : ''}
        ${!isNum ? `<div style="display:flex;flex-direction:column;gap:2px">
          ${['A','B','C','D'].map(k => {
            const v = q['option_' + k.toLowerCase()];
            const oImg = q['option_' + k.toLowerCase() + '_image_path'];
            if (!v && !oImg) return '';
            return `<div style="font-size:11px;color:${txtClr}">
              <span style="font-weight:700;min-width:16px;display:inline-block">(${k})</span> ${v || ''}
              ${oImg ? `<img src="${_imgUrl(oImg)}" style="max-width:90%;height:auto;max-height:80px;display:block;margin-top:2px;border-radius:3px" onerror="this.style.display='none'">` : ''}
            </div>`;
          }).join('')}
          ${q.options_image_path ? `<img src="${_imgUrl(q.options_image_path)}" style="max-width:100%;height:auto;max-height:180px;margin-top:4px;border-radius:4px;border:1px solid ${isNEET ? '#c8b06a' : '#cbd5e1'}" onerror="this.style.display='none'">` : ''}
...
// ─── STANDARD QUESTION RENDER (THE MAIN FIX) ────────────────────────────────
// Fixes:
//  1. Per-option images (option_a_image_path etc) rendered below each option text
//  2. options_image_path (combined image) shown as its own block ABOVE individual options
//  3. Selected option state fully cleared before re-setting — no stuck blue border
function _renderQ(q) {
  ...
  if (q.question_image_path) qHtml += _imgTag(q.question_image_path);
  if (!qHtml)                qHtml  = `<div class="q-text" style="color:var(--c-text4);font-style:italic">No question text</div>`;

  // ── Options block ─────────────────────────────────────────
  // If there's a SINGLE combined options image (e.g. all options printed as one image),
  // show it in its own block. Per-option images go inside each option row.
  let ansHtml = '';
  if (isNum) {
    const curVal = ans.sel !== null && ans.sel !== '' ? ans.sel : '—';
    ansHtml = `<div class="numpad-wrapper">
      <div class="num-display" id="num-disp">${curVal}</div>
      <div class="numpad-grid">
        ${[7,8,9,'DEL',4,5,6,'±',1,2,3,'.','0','00','C','OK'].map(k =>
          `<button class="numpad-key ${k==='DEL'||k==='C'?'del':k==='±'||k==='OK'?'fn':''}" onclick="examNumKey(${q.id},'${k}')">${k}</button>`
        ).join('')}
      </div>
    </div>`;
  } else {
    // Combined options image (if admin uploaded one image for all 4 options)
    const combinedImg = q.options_image_path
      ? `<div style="margin-bottom:10px;text-align:center">
           <img src="${_imgUrl(q.options_image_path)}"
             style="max-width:100%;height:auto;max-height:260px;border-radius:8px;border:1px solid var(--c-border);display:block;margin:0 auto"
    ...
    const optRows = ['A','B','C','D'].map(k => {
      const v    = q['option_' + k.toLowerCase()];
      const oImg = q['option_' + k.toLowerCase() + '_image_path'];
      if (!v && !oImg) return '';   // skip empty options
      const isSel  = selSet.has(k);
      const isCorr = corr.has(k);
      // In review mode: green = correct, red = wrong-selected, grey = unselected
      let cls = '';
      if (ro) {
      ...
      return `<div class="option-row ${cls}" data-opt="${k}" ${ro ? 'style="cursor:default"' : ''} ${click}>
        <div class="option-key">${k}</div>
        <div class="option-text" style="flex:1">
          ${v || ''}
          ${oImg ? `<img src="${_imgUrl(oImg)}"
            style="display:block;max-width:100%;height:auto;max-height:120px;margin-top:6px;border-radius:6px;border:1px solid var(--c-border)"
            onerror="this.style.display='none'"
            alt="Option ${k} image">` : ''}
        </div>
        ${ro && isCorr ? `<span style="margin-left:auto;color:var(--c-green);flex-shrink:0">${IC.chk}</span>` : ''}
      </div>`;
    }).join('');

    ansHtml = `<div class="options-list">${combinedImg}${optRows}</div>`;
  }

  // ── Solution (review mode only) ───────────────────────────
  let solHtml = '';
  if (ro) {
    solHtml = `<div style="margin-top:14px;padding:12px;background:var(--c-green-l);border-radius:var(--radius-sm);border-left:3px solid var(--c-green)">
      <div style="font-size:10px;font-weight:800;text-transform:uppercase;color:var(--c-green);margin-bottom:6px">Answer: ${q.correct_answer || '—'}</div>
      ${q.solution_text ? `<div style="font-size:12px;color:var(--c-text2);line-height:1.7">${q.solution_text}</div>` : ''}
      ${q.solution_image_path ? _imgTag(q.solution_image_path) : ''}
    </div>`;
  }
...
// ─── OPTION PICK (standard mode) — clears ALL before setting new ────────────
// FIX: instead of relying on class-toggling which can leave stale state,
// we re-render the whole question which is the single source of truth.
window.examPickOpt = function(qId, key, isMulti) {
  ...
  if (isMulti) {
    ...
    // Toggle off if same, otherwise set new. Always a clean string.
    ans.sel = (ans.sel === key) ? null : key;
  }
  ans.status = ans.sel ? 'ANSWERED' : 'NOT_ANSWERED';
  _sendAns(qId);
  // Re-render the WHOLE question card — eliminates any stale CSS state
...
function _renderFooter(q) {
  ...
  const ES = window._ES, ro = ES.readonly;
  const f = ES.cur === 0, l = ES.cur === ES.qs.length - 1;
  if (ro) {
    ...
    el.innerHTML = `
      ...
      <button class="btn btn-sm" style="background:var(--q-marked);color:#fff;border:none" onclick="window._markR(${q.id})">Mark & Review</button>
...
function _renderPalette() {
  ...
  leg.innerHTML = `
    <div class="legend-row"><div class="legend-dot ans"></div>${cnt.ANSWERED} Ans</div>
    <div class="legend-row"><div class="legend-dot na"></div>${cnt.NOT_ANSWERED} NA</div>
    <div class="legend-row"><div class="legend-dot mrk"></div>${cnt.MARKED_FOR_REVIEW + cnt.ANSWERED_AND_MARKED} Mrk</div>
    <div class="legend-row"><div class="legend-dot nv"></div>${cnt.NOT_VISITED} NV</div>`;
  const byS = {}, sOrd = [];
  ...
  grid.innerHTML = sOrd.map(s =>
    `<div class="palette-subj-label">${s.charAt(0) + s.slice(1).toLowerCase()}</div>
     <div class="palette-buttons">${byS[s].map(({ q, i }) => {
...
// ─── OMR POPUP (standard mode) ───────────────────────────────────────────────
window._omrOpen = function() {
  if (window._ES.qs.length) OMR.open(window._ES.qs, window._ES.answers, function(qId, ans) {
    window._ES.answers[qId] = ans;
    _sendAns(qId); _renderPalette();
    const c = window._ES.qs[window._ES.cur];
    if (c && c.id === qId) _renderQ(c);
  });
};
    ...
    if (mcqs.length) {
      ...
      mcqs.forEach(q => {
        ...
        h += `<div style="display:flex;align-items:center;gap:3px;padding:2px 0;border-bottom:1px dotted #e0c97a">
          ...
          ${['A','B','C','D'].map(o => {
            const f = sel.has(o);
            return `<div onclick="omrBubbleClick(${q.id},'${o}',${isMulti})"
              style="width:20px;height:20px;border-radius:50%;border:1.5px solid ${f?'#2c1a00':'#c8a000'};background:${f?'#2c1a00':'transparent'};color:${f?'#fff9e6':'#5c3d00'};display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:900;cursor:pointer;user-select:none;transition:all .1s;flex-shrink:0">${o}</div>`;
          }).join('')}
        </div>`;
      });
      h += `</div>`;
...
function _renderJeeOMRPanel() {
  ...
  let h = `<div style="font-family:'Courier New',monospace;font-size:10px">
    <div style="text-align:center;font-size:9px;font-weight:800;letter-spacing:.1em;color:#94a3b8;border:1px solid #334155;padding:3px;margin-bottom:8px;border-radius:2px">JEE OMR RESPONSE SHEET</div>`;
  sOrd.forEach(subj => {
    const mcqs  = bySubj[subj].filter(q => q.question_type !== 'NUMERICAL');
    const nums   = bySubj[subj].filter(q => q.question_type === 'NUMERICAL');
      ...
      mcqs.forEach(q => {
        ...
        h += `<div style="display:flex;align-items:center;gap:2px;padding:2px 0;border-bottom:1px solid #0f172a;margin-bottom:1px">
          ...
          ${['A','B','C','D'].map(o => {
            const f = sel.has(o);
            return `<div onclick="omrBubbleClick(${q.id},'${o}',${isMulti})"
              style="width:17px;height:17px;border-radius:50%;border:1.5px solid ${f?'#60a5fa':'#334155'};background:${f?'#60a5fa':'transparent'};color:${f?'#0f172a':'#64748b'};display:flex;align-items:center;justify-content:center;font-size:7px;font-weight:900;cursor:pointer;flex-shrink:0">${o}</div>`;
          }).join('')}
        </div>`;
      });
    }
...
function _numericOMRBoxes(qId, val, neetStyle) {
  ...
  const filled = neetStyle ? '#2c1a00' : '#1d4ed8';
  const filledTxt = neetStyle ? '#fff9e6' : '#fff';
  const empty = 'transparent', emptyTxt = neetStyle ? '#c8a000' : '#64748b';
  const box = (content, isFilled, onclick) =>
    `<div onclick="${onclick}" style="width:${neetStyle?'16px':'15px'};height:${neetStyle?'20px':'18px'};border:${neetStyle?'1.5px solid #c8a000':'1px solid #334155'};border-radius:2px;background:${isFilled?filled:empty};color:${isFilled?filledTxt:emptyTxt};display:flex;align-items:center;justify-content:center;font-size:${neetStyle?'9px':'8px'};font-weight:900;cursor:pointer;flex-shrink:0">${content}</div>`;
  return `
    ${box(neg ? '−' : '+', neg, `omrNumSign(${qId})`)}
    ${ip.map((d, i) => box(d || '·', !!d, `omrNumDigit(${qId},'int',${i})`)).join('')}
    <div style="font-size:9px;font-weight:900;color:${neetStyle?'#5c3d00':'#64748b'};padding:0 1px">.</div>
    ${dp.map((d, i) => box(d || '·', !!d, `omrNumDigit(${qId},'dec',${i})`)).join('')}
  `;
}

// Global OMR bubble handlers
...
async function _camStart() {
  ...
  vid.id = '_ep_cam';
  vid.style.cssText = 'position:fixed;bottom:72px;right:12px;width:110px;height:82px;border-radius:8px;border:2px solid var(--c-green);z-index:2000;object-fit:cover;box-shadow:0 4px 14px rgba(0,0,0,.35)';
...
window._submitConfirm = function() {
  ...
  const cnt = { ANSWERED: 0, NOT_ANSWERED: 0, NOT_VISITED: 0, MARKED_FOR_REVIEW: 0 };
  Object.values(ES.answers).forEach(a => { if (a && cnt[a.status] !== undefined) cnt[a.status]++; });
  openModal('Submit Test', `<div style="text-align:center;padding:8px">
  ...
  `<button class="btn btn-secondary" onclick="closeModal()">Continue</button><button class="btn btn-danger" onclick="closeModal();_doSubmit(false)">Submit Now</button>`);
};
...
async function _doSubmit(auto) {
  ...
  try {
    ...
    ov.style.display = 'none'; _showResult(r);
...
function _showResult(r) {
  ...
  if (r.subject_breakdown && Object.keys(r.subject_breakdown).length) {
    bk = `<div class="card" style="margin-top:16px"><div class="card-body"><div style="font-size:12px;font-weight:800;color:var(--c-text);margin-bottom:12px">Subject Breakdown</div>
      ...
      <tbody>${Object.entries(r.subject_breakdown).map(([s, b]) =>
        `<tr><td style="font-weight:600">${s.charAt(0)+s.slice(1).toLowerCase()}</td><td style="color:var(--c-green);font-weight:700">${b.correct||0}</td><td style="color:var(--c-red);font-weight:700">${b.incorrect||0}</td><td style="color:var(--c-text3)">${b.unattempted||0}</td><td style="text-align:right;font-weight:800;color:var(--c-blue)">${Number(b.score||0).toFixed(1)}</td></tr>`
